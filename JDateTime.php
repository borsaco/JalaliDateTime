...
namespace Borsaco\jalaliDateTime;
...
class JDateTime
{
	 ...
	 * @param bool|int $stamp int Unix Timestamp (Epoch Time)
    ...
    public static function date($format, $stamp = false, $convert = null, $jalali = null, $timezone = null)
    {
	    ...
	    {
		    $timezone = ( $timezone != null ) ? $timezone : self::$timezone;
	    }
	    else
	    {
		    $timezone = ( $timezone != null ) ? $timezone : date_default_timezone_get();
	    }
...
//Find what to replace
	    ...
	    //Changed Keys
	    [$year, $month, $day] = array($obj->format('Y'), $obj->format('n'), $obj->format('j'));
	    ...
	    foreach ($keys as $k => $key) {
	        ...
	        switch ($key) {
	            ...
	            //Week
	            case 'W':
	                $v = is_int(self::$temp['z'] / 7) ? (self::$temp['z'] / 7) : (int) ( self::$temp['z'] / 7 + 1 );
	                break;
	            //Month
	            case 'F':
   ...
   /**
    ...
    {
        //Defaults
        $month = ( (int) $month == 0) ? self::date('m') : $month;
        $day   = ( (int) $day == 0) ? self::date('d') : $day;
        $year  = ( (int) $year == 0) ? self::date('Y') : $year;
        $hour  = (int) $hour;
        $minute  = (int) $minute;
        $second  = (int) $second;
    ...
    {
        //Defaults
        $month = ( (int) $month == 0) ? self::date('n') : (int) $month;
        $day   = ( (int) $day == 0) ? self::date('j') : (int) $day;
        $year  = ( (int) $year == 0) ? self::date('Y') : (int) $year;
        ...
        if ( $jalali === true || ($jalali === null && self::$jalali === true) )
        {
	        ...
	        $ret = self::date( 'Y-n-j', $epoch, false ) == "$year-$month-$day";
        }
        else //Gregorian Date
        { 
	 ...
	 * @return array
	 */
    private static function filterArray($needle, $heystack, array $always = array() )
    {
    ...
    {
	    return function_exists( 'mb_substr' ) ? mb_substr( $str, $start, $len, 'UTF-8' ) : substr( $str, $start, $len * 2 );
    }
